...
import procyk.industries.engines.ChatEngine;
import procyk.industries.engines.SkillsEngine;
import procyk.industries.engines.StatisticsEngine;
import procyk.industries.engines.SkillsEngine.SkillType;
import procyk.industries.gui.TrainSkillGui;
...
public class TrainSkill extends PollingScript<ClientContext> implements MessageListener, PaintListener{
	...
	{
		if(procyk.industries.localplayer.Location.Within(ctx, bankArea) && procyk.industries.localplayer.Backpack.hasStuff(ctx) && isBanking)
		{
			System.out.println("Depositing");
			TrainSkillGui.GetInstance().AppendDebugText("Depositing");
			return State.deposit;
		}
		else if(procyk.industries.localplayer.Location.Within(ctx, bankArea) && !procyk.industries.localplayer.Backpack.hasStuff(ctx))
		{
			System.out.println("Walking to site");
			TrainSkillGui.GetInstance().AppendDebugText("Walking to site");
			return State.walk_to_site;
		}
		else if(procyk.industries.localplayer.Location.Within(ctx, siteArea) && !procyk.industries.localplayer.Backpack.isFull(ctx))
		{
