...
@Manifest(name = "Train Skills", description = "Train Mining, Fishing, Woodcutting in various locations", properties = "client=4; topic=0;")
public class TrainSkill extends PollingScript<ClientContext> implements MessageListener, PaintListener{
	...
	{
		if(Location.Within(ctx, bankArea) &&Backpack.hasStuff(ctx) && isBanking)
		{
		...
		else if(Location.Within(ctx, siteArea) && !Backpack.isFull(ctx))
		{
