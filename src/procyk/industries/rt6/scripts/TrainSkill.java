...
public class TrainSkill extends PollingScript<ClientContext> implements MessageListener, PaintListener{
	...
	{
		if(procyk.industries.rt6.localplayer.Location.Within(ctx, bankArea) && procyk.industries.rt6.localplayer.Backpack.hasStuff(ctx) && isBanking)
		{
			System.out.println("Depositing");
			TrainSkillGui.GetInstance().AppendDebugText("Depositing");
			return State.deposit;
		}
		else if(procyk.industries.rt6.localplayer.Location.Within(ctx, bankArea) && !procyk.industries.rt6.localplayer.Backpack.hasStuff(ctx))
		{
			System.out.println("Walking to site");
			TrainSkillGui.GetInstance().AppendDebugText("Walking to site");
			return State.walk_to_site;
		}
		else if(procyk.industries.rt6.localplayer.Location.Within(ctx, siteArea) && !procyk.industries.rt6.localplayer.Backpack.isFull(ctx))
		{
