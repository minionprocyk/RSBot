package procyk.industries.rt6.engines;
...
import procyk.industries.rt6.manager.AvoidObject;
import procyk.industries.rt6.manager.AvoidObjectsManager;
	...
	public void run() {
		...
		else
		{
			...
			if(Location.DistanceTo(ctx, site) < 100)
			{
				...
				procyk.industries.rt6.pathing.MoveTowards.Location(ctx, site);
				return; //we tried to fix the problem. try again
			}
			else
			{
				System.out.println("The site is too far away. Walk to it");
				procyk.industries.rt6.camera.Focus.OnObject(ctx, site.getCentralTile());
				ctx.controller.stop();
				return;
			}
		}
		...
		try
			...
			if(objectNames == null)
			{
				collection.addAll(procyk.industries.rt6.gameobjects.Select.WithinArea(ctx, site, objectIds));
			}
			else
			{
				collection.addAll(procyk.industries.rt6.gameobjects.Select.WithinArea(ctx, site, objectNames));
			}
			...
			if(gameObject.valid())
			{
				...
				if(procyk.industries.rt6.tiles.Calculations.isPlayerNearTile(ctx, gameObject.tile()))
				{
